...
class TestCheckExplicitTableNames(SimpleTestCase):
    ...
    def test_error_when_explicit_names_required(self):
        errors = CheckExplicitTableNames()(self.app_configs)
    ...
    def test_no_error_when_app_is_ignored(self):
        check = CheckExplicitTableNames(ignore_labels=['checks_explicit_table_names'])
        errors = check(self.app_configs)
        self.assertEqual(len(errors), 0)

    def test_no_error_when_model_is_ignored(self):
        check = CheckExplicitTableNames(ignore_labels=['checks_explicit_table_names.WithTableNameUpperCase'])
        errors = check(self.app_configs)
